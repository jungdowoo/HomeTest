...
const faqs = [
  {
    ...
    a: "아닙니다. 결과는 입력값과 룰 엔진에 따른 참고 정보입니다. 실제 신청 가능 여부는 반드시 청약홈과 해당 단지 모집공고문으로 확인해야 하며, 결과 화면의 기준일과 룰 버전도 함께 보는 것이 좋습니다.",
  },
  {
    q: "AI가 청약 자격을 판단하나요?",
    a: "아닙니다. 자격 판정은 코드 기반 룰 엔진이 수행합니다. AI는 계산된 결과를 쉬운 문장으로 설명하는 역할만 하고, 부족 조건을 다시 읽기 쉽게 풀어주는 보조 역할을 맡습니다.",
  },
  {
    q: "무주택 기간이 짧으면 왜 특별공급을 추천하나요?",
    a: "무주택 기간이 짧으면 일반공급 가점 경쟁에서 불리할 수 있기 때문입니다. 다만 혼인, 생애최초, 자녀 수 등 특별공급 요건과 단지 공급 유형이 함께 맞아야 하므로, '특공 우선 검토'는 출발점이지 최종 결론은 아닙니다.",
  },
  {
    q: "예상 경쟁률은 실제 경쟁률인가요?",
    a: "아닙니다. 현재 MVP의 예상 경쟁 강도는 지역, 주택 유형, 모집 상태 등을 바탕으로 만든 참고용 분류입니다. 실제 경쟁률은 청약홈, 공식 공고, 사업주체 안내를 확인해야 하며, 이 값은 비교를 돕는 보조 지표에 가깝습니다.",
  },
  {
    q: "소득이나 자산을 비워두면 왜 주의가 뜨나요?",
    a: "소득·자산 기준은 전형과 공고에 따라 달라질 수 있어 비어 있는 경우 확정 판단이 어렵습니다. 그래서 추가 확인이 필요하다는 의미로 주의 상태를 표시합니다. 가능하면 최근 소득 자료와 자산 관련 증빙을 준비한 뒤 다시 확인하는 편이 좋습니다.",
  },
  {
    q: "선택한 단지에 없는 특별공급도 추천되나요?",
    a: "아닙니다. 선택한 단지에서 제공하지 않는 공급 유형은 추천 순위에서 제외하고 '공급 유형 없음'으로 표시합니다. 개인 조건이 맞더라도 단지에서 제공하지 않으면 실제 선택지로 볼 수 없습니다.",
  },
  {
    q: "개인정보가 저장되나요?",
  ...
  },
  {
    ...
    a: "그럴 수 있습니다. 실제 공고에서는 지역 우선공급이나 거주기간 요건이 영향을 줄 수 있으므로, 서비스는 이를 주의 항목으로 안내하고 최종 확인을 권장합니다. 같은 조건이라도 지역이 달라지면 추천 순위가 바뀔 수 있습니다.",
  },
  {
  ...
  {
    q: "어떤 페이지를 먼저 읽으면 서비스 이해가 빠를까요?",
    a: "가이드, 서비스 소개, 그리고 '왜 청약 자격을 AI에게 직접 판정시키지 않을까' 블로그 글부터 보면 구조를 이해하기 쉽습니다. 그 다음에 단지 목록과 추천 결과를 보면 화면이 왜 그렇게 구성됐는지 자연스럽게 보입니다.",
  },
  {
    q: "이 서비스는 앞으로 무엇으로 확장될 수 있나요?",
    a: "Supabase를 붙여 룰 버전 관리, 즐겨찾기 저장, 사용자 입력 캐시를 둘 수 있고, OpenAI API로 설명 품질을 높일 수도 있습니다. 관리자 룰 관리 페이지를 만들면 청약 정책 변경도 더 쉽게 반영할 수 있습니다.",
  },
];
